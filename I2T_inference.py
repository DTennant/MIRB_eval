...
mcq_tasks = ['codeu', 'foods', 'image_jigsaw', 'codeu_text']
generation_tasks = ['analogy', 'count', 'arxiv', 'domain', 'image_needles', 'plot', 'places', "3d_scene", '3d_scene_concat', 'image_needles_concat', 'count_concat',
...
def parse_args():
    ...
    parser.add_argument('--dataset', default=['count'], choices=['analogy', 'count', 'arxiv', 'domain', 'image_needles', 'plot', 'codeu', 'places', 'foods', 'image_jigsaw', "3d_scene"
                                                                 , '3d_scene_concat', 'image_needles_concat', 'count_concat', 'plot_text', 'arxiv_text', 'codeu_text'], nargs="+")
    parser.add_argument("--engine", "-e", choices=["openflamingo", "otter-llama", "llava15-7b", "llava16-7b", "llava16-13b", "qwen-vl", "qwen-vl-chat", 'internlm-x2', 
                                                   'emu2-chat', 'idefics1-9b-instruct', 'idefics1-80b-instruct', 'idefics2-8b', 'gpt4v-openai', 'gpt4v-azure', 'vila-7b', 'vila-2.7b',
                                                   'internlm-x2-hd', "phi3-vision", "gemini-pro"],
...
def eval_gen(args, query_meta, model, tokenizer, processor, engine, dataset):
    ...
    for query in query_meta:
        if args.single:
            query['images'] = [random.choice(query['images'])]
...
if __name__ == "__main__":
    ...
    for engine in args.engine:
        ...
        for dataset in args.dataset:
            ...
            if dataset in mcq_tasks and engine not in ['gpt4v', 'emu2-chat', 'gpt4v-azure']:
                results_dict = eval_mcq(args, query_meta, model, tokenizer, processor, engine, dataset)
            elif dataset in generation_tasks or engine in ['gpt4v', 'emu2-chat', 'gpt4v-azure']:
            ...
            if args.CoT:
                ...
                result_path = f"results/{dataset}/{engine}_single.json"
            else:
